...
public class HandshakePubSubType implements us.ihmc.pubsub.TopicDataType<us.ihmc.robotDataLogger.Handshake>
{
   ...
   public final static int getCdrSerializedSize(us.ihmc.robotDataLogger.Handshake data, int current_alignment)
   {
      ...
      {
          current_alignment += us.ihmc.robotDataLogger.YoRegistryDefinitionPubSubType.getCdrSerializedSize(data.getRegistries().get(i0), current_alignment);}

      current_alignment += 4 + us.ihmc.idl.CDR.alignment(current_alignment, 4);
      for(int i0 = 0; i0 < data.getVariables().size(); ++i0)
      {
          current_alignment += us.ihmc.robotDataLogger.YoVariableDefinitionPubSubType.getCdrSerializedSize(data.getVariables().get(i0), current_alignment);}

      current_alignment += 4 + us.ihmc.idl.CDR.alignment(current_alignment, 4);
      for(int i0 = 0; i0 < data.getJoints().size(); ++i0)
      {
          current_alignment += us.ihmc.robotDataLogger.JointDefinitionPubSubType.getCdrSerializedSize(data.getJoints().get(i0), current_alignment);}
